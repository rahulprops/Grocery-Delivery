      ...
      <div className=" fixed right-0 top-0 h-full w-full max-w-md bg-white z-50 shadow-2xl flex flex-col  animate-slide-in-right">
        ...
        <div className=" flex  items-center justify-between p-5 border-b border-app-border">
          <div className=" flex items-center gap-2">
            <ShoppingBagIcon className=" sixe-5" />
              ...
              >
                ...
                <div className=" flex-1 min-w-0">
                  ...
                  <div className=" flex items-center justify-between mt-2">
                    ...
                    <div className=" flex items-center gap-2">
                        <span className=" text-sm font-semibold">{currency} {(item.product.price * item.quantity)}</span>
          ...
          {items.length > 0 && (
            <div className=" p-5 border-t border-app-border space-y-3 ">
                ...
                <div className=" flex justify-between text-sm">
                    ...
                    <span className=" font-medium">{deliveryFee === 0 ? <span className=" text-app-success"> Free</span> : <span>
                         `${currency} ${deliveryFee.toFixed(2)}`
                        </span>}</span>

                </div>
                {deliveryFee > 0 && <p className=" text-xs text-app-text-light text-center"> Free delivery on orders over ${currency} 50!</p>}
                ...
                <button onClick={()=>{ setIsCartOpen(false); navigate('/checkout'); window.scrollTo(0,0)}} className="  w-full py-3 bg-app-orange text-white font-semibold rounded-xl hover:bg-app-orange-dark transition-colors flex-center gap-2 active:scale-[0,98]" >
                    Proceed to chekout <ArrowRightIcon  className=" size-4"/>
                </button>
            </div>
          )}
