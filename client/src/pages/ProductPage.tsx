...
import { ArrowLeftIcon, ArrowRightIcon, HomeIcon, LeafIcon, MinusIcon, PlusIcon, Section, ShoppingCartIcon, StarIcon } from "lucide-react";
...
const ProductPage = () => {
  ...
  const [localQuantity, setLoaclQuantity] = useState(1);
  ...
  const handleMinus = ()=>{
    if(inCart){
      ...
      setLoaclQuantity(Math.max(1,localQuantity-1))
    }
  }

  const handlePlus = ()=>{
    if(inCart) updateQuantity(product._id , cartItem.quantity +1)
      else setLoaclQuantity(localQuantity +1)
  }
        ...
        { relatedProducts.length > 0 && (
           <section className=" mt-12 mb-44">
              <div className=" flex items-center justify-between mb-6">
                ...
                <Link to={`/products?category={product.category}`} className=" text-sm font-semibold text-app-orange hover:text-shadow-app-orange-dark flex items-center gap-1 transition-colors">
