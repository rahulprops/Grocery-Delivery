...
const MyOrders = () => {
  const current = import.meta.env.VITE_CURRENCY_SYMBOM || "$";
        ...
        <div className=" flex gap-2 mb-6 overflow-x-auto pb-2">
          {tabs.map((tab) => (
            <button
              ...
              className={` px-4 py-2 text-sm font-medium rounded-xl whitespace-normal transition-colors ${activeTab === tab ? " bg-app-green text-white " : " bg-white text-app-text-light hover:bg-app-cream"}`}
            >
                ...
                <div className=" flex  items-center gap-2 mb-3">
                  {order.items.slice(0, 4).map((item, i) => (
                    <img
                     ...
                     rounded-lg object-cover border border-app-border"
                    />
                  ))}
